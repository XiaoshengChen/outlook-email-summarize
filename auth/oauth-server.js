...
function setupOAuthRoutes(app, tokenStorage, authConfig, envPrefix = 'MS_') {
  ...
  app.get('/auth/callback', async (req, res) => {
    ...
    if (!pendingStates.has(state)) {
      console.error('OAuth callback state mismatch. Rejecting request to prevent potential CSRF attack.');
      return res.status(400).send(templates.authError('Invalid State Parameter', 'The OAuth state parameter did not match the issued value. Please try authenticating again.'));
    }
